import React, { useState, useEffect, useContext } from '@domains/Dapp/views/Pools/Pools/incentivizers/DegovDailp/react';

import useSWR from '@domains/Dapp/views/Pools/Pools/incentivizers/DegovDailp/swr';
import { formatEther, formatUnits, parseEther, parseUnits } from '@domains/Dapp/views/Pools/Pools/incentivizers/DegovDailp/ethers/lib/utils';
import { Contract } from '@domains/Dapp/views/Pools/Pools/incentivizers/DegovDailp/ethers';
import { useWeb3React } from '@domains/Dapp/views/Pools/Pools/incentivizers/DegovDailp/@web3-react/core';
import { contractAddress, poolAbi, lpAbi, fetcher } from '@domains/Dapp/views/Pools/Pools/incentivizers/DegovDailp/utils';

/* import components */
import { Button, Input, Flexbox } from '@domains/Dapp/views/Pools/Pools/incentivizers/DegovDailp/domains/Dapp/views/DashboardView/components/common';
import { PoolStake } from '@domains/Dapp/views/Pools/Pools/incentivizers/DegovDailp/domains/Dapp/views/DashboardView/components/layout';
/* import context */
import { SnackbarContext } from '@domains/Dapp/views/Pools/Pools/incentivizers/DegovDailp/domains/Dapp/views/DashboardView/components/common';
...
const DegovDailpStake = ({ info }) => {
    ...
    /* static data */
    const rewardTokenAddress = contractAddress.degov;
    const stakeTokenAddress = contractAddress.debaseDaiLp;
    const poolAddress = contractAddress.degovDaiLpPool;
    ...
    const listData = [
        {
            label: 'Balance',
            value: rewardBalance ? parseFloat(formatEther(rewardTokenBalance)).toFixed(8) * 1 + ' Degov' : '0 Degov',
            valueType: 'degov'
        },
        {
            label: 'Claimable',
            value: percents ? rewardBalance && tokenSupply ? parseFloat(formatEther(rewardBalance.mul(tokenSupply).div(parseEther('1')))).toFixed(8) * 1 + ' Degov' : '0 Degov' : rewardBalance ? parseFloat(formatEther(rewardBalance)).toFixed(8) * 1 + ' Degov' : '0 Degov',
            valueType: 'degov'
        },
        {
            label: 'To stake',
